#!/usr/bin/env python3
"""
Data Generation Script (V6.1 – Debugged SKU in Shipments)

Generates synthetic data for a multi-hub apparel network:
 - 3 stores/major city, 2 stores/minor city
 - Partial fulfillment + auto-reorder
 - Inventory-driven reorders below threshold
 - Seasonality & multipliers in demand
 - Final tables: dims (hubs, stores, products, suppliers)
               facts (orders, shipments, returns,
                     inventory snapshots, unfulfilled orders, inbound shipments)
"""
import logging
import random
...
# -----------------------------------------------------------------------------
# 4. FACTS: ORDERS, SHIPMENTS, INBOUNDS, INVENTORY, RETURNS
# -----------------------------------------------------------------------------
def sim_orders(
    stores: pd.DataFrame,
...
def sim_shipments(
    ...
    # Seed hub–SKU inventory
    inv = {(hub, sku): random.randint(300,1500)
    ...
    unful_df   = pd.DataFrame(unfilled)
...
def sim_inbound(
    ...
    for i in range(1, count+1):
        ...
        if inv < reorder_threshold:
            ...
            delay = (timedelta(days=random.randint(1,5))
...
def sim_reorders(
    ...
    hub_map = hubs.set_index("Specialty")["Hub_ID"].to_dict()
    recs = []
    for _, u in unful_df.iterrows():
        prod = products.loc[products.SKU==u.SKU].iloc[0]
        hub  = hub_map[prod.Category]
        sup  = sup_full.sample(1).iloc[0]
        prom = fake.date_time_between(CFG["START_DATE"], CFG["END_DATE"])
        delay = (timedelta(days=random.randint(1,5))
        ...
        recs.append({
            "Inbound_Shipment_ID":  f"INB_REO_{u.Order_Line_ID}",
            "Supplier_ID":          sup.Supplier_ID,
            "Destination_Hub_ID":   hub,
            "SKU":                  u.SKU,
            "Quantity_Received":    u.Unfulfilled_Quantity,
            "Promised_Arrival_Date": prom.date(),
            "Actual_Arrival_Date":   (prom + delay).date()
        })

    df = pd.DataFrame(recs)
    log.info("Reorders generated: %d", len(df))
    return df
...
def sim_inventory(
    ...
    inv_levels = {(hub, sku): random.randint(200,800)
                  for hub in hubs.Hub_ID for sku in products.SKU}
    ...
    df = pd.DataFrame(records)
    log.info("Inventory snapshots: %d", len(df))
    return df
...
def sim_returns(
    ...
    df = (sample.merge(orders, on="Order_Line_ID")
...
def main():
    ...
    inv_levels = {(hub.Hub_ID, sku): random.randint(200,800)
    ...
    reorders_df = sim_reorders(unful_df, sup_full, products, hubs)
    initial_inb  = sim_inbound(sup_full, products, hubs, inv_levels)

    all_inbound = pd.concat([initial_inb, reorders_df], ignore_index=True)
